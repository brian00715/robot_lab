...
# Original: go2_gym/envs/base/curriculum.py

import numpy as np


class Curriculum:
    """Base curriculum class using a grid of command parameters."""
    ...
    def set_to(self, low, high, value=1.0):
        ...
        assert len(inds) != 0, "Initializing distribution with empty domain!"
    ...
    def __init__(self, seed, **key_ranges):
        ...
        self.lows = np.array([r[0] for r in key_ranges.values()])
        self.highs = np.array([r[1] for r in key_ranges.values()])
        ...
        self._l = len(self.grid[0])
        self.ls = {key: len(self.cfg[key]) for key in self.cfg.keys()}
        self.weights = np.zeros(self._l)
        self.indices = np.arange(self._l)
...
class RewardThresholdCurriculum(Curriculum):
    """Curriculum that expands based on reward thresholds being met."""
